...
async function findOne(req, res) {
    try {
        ...
        const productFound = await db.em.findOneOrFail(Product, {
            id,
        }, { populate: ['category', 'tags', 'orders'] });
    ...
    try {
        ...
        if (req.body.tagIds && Array.isArray(req.body.tagIds)) {
            ...
            if (tags.length !== tagIds.length) {
                ...
                const nonExistingTagIds = tagIds.filter((id) => !existingTagIds.includes(id));
...
async function update(req, res) {
    try {
        const id = Number.parseInt(req.params.id);
        const productToUpdate = await db.em.findOne(Product, { id }, { populate: ['tags'] });
        if (!productToUpdate) {
...
async function findOutOfStock(req, res) {
    try {
...
async function actualizarStockMasivo(req, res) {
...
export { findAll, findOne, add, update, remove, findByCategory, findOutOfStock, actualizarStockMasivo, };
